...
if Config.Options.NotHealthRecharge then
	SetPlayerHealthRechargeMultiplier(PlayerId(), 0.0)
end

if Config.Options.MuteAmbience then
	StartAudioScene('CHARACTER_CHANGE_IN_SKY_SCENE')
end

SetBlackout(Config.Options.Blackout)
					...
					if GetPedSourceOfDeath(entity) == PlayerPedId() then
						...
						if not IsPedInAnyVehicle(PlayerPedId(), false) then
							if(Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 1.5) then
								ESX.Game.Utils.DrawText3D({x = pedX, y = pedY, z = pedZ + 0.2}, DE.press_to_loot_entitys, 0.8, 4)
								if IsControlJustReleased(1, 51) then
									if DoesEntityExist(GetPlayerPed(-1)) then
										...
										elseif randomChance >= Config.ProbabilityItemLoot and randomChance < 100 then
											ESX.ShowNotification(De.nothing_found)
										end
